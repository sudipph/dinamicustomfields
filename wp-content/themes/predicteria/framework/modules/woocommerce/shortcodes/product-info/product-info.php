<?php
namespace WellExpoCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Product Info', 'wellexpo' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'wellexpo' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'wellexpo' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'wellexpo' ),
							'value'       => array(
								esc_html__( 'Title', 'wellexpo' )          => 'title',
								esc_html__( 'Featured Image', 'wellexpo' ) => 'featured_image',
								esc_html__( 'Category', 'wellexpo' )       => 'category',
								esc_html__( 'Excerpt', 'wellexpo' )        => 'excerpt',
								esc_html__( 'Price', 'wellexpo' )          => 'price',
								esc_html__( 'Rating', 'wellexpo' )         => 'rating',
								esc_html__( 'Add To Cart', 'wellexpo' )    => 'add_to_cart',
								esc_html__( 'Tag', 'wellexpo' )            => 'tag',
								esc_html__( 'Author', 'wellexpo' )         => 'author',
								esc_html__( 'Date', 'wellexpo' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'wellexpo' ),
						...
						array(
							...
							'description' => esc_html__( 'Set title tag for product title element', 'wellexpo' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'wellexpo' ),
							'value'      => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'wellexpo' ),
							'value'       => array_flip( wellexpo_select_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'wellexpo' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'wellexpo' ),
							'value'      => array(
								esc_html__( 'Default', 'wellexpo' ) => '',
								esc_html__( 'White', 'wellexpo' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'wellexpo' ),
							'value'      => array(
								esc_html__( 'Default', 'wellexpo' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'wellexpo' ) ),
							'group'      => esc_html__( 'Product Info Style', 'wellexpo' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="qodef-pi-title entry-title" ' . wellexpo_select_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="qodef-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . wellexpo_select_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="qodef-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . wellexpo_select_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="qodef-pi-excerpt"><p itemprop="description" class="qodef-pi-excerpt-item" ' . wellexpo_select_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="qodef-pi-price" ' . wellexpo_select_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="qodef-pi-rating" title="' . sprintf( esc_attr__( "Rated %s out of 5", "wellexpo" ), $average ) . '" ' . wellexpo_select_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'wellexpo_select_filter_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="qodef-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="qodef-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . wellexpo_select_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="qodef-pi-author" ' . wellexpo_select_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="qodef-pi-date" ' . wellexpo_select_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'wellexpo' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'wellexpo' ) . ': ' . $product_id;
