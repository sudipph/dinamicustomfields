			...
			for(var prop in tt_fields) {
				...
				if(tt_atts[prop].val!=null)
				{
					...
					if(["font"].indexOf(field.type)!==-1)
						$field.val(tt_atts[prop].val).trigger("change", (tt_atts['font_subset']!=null ? [tt_atts['font_subset'].val.split(",")] : null));
...
function generateShortcode()
{
	...
	$(".tt_shortcode").val("[tt_timetable" + 
		...
		($("#available_slots_singular_label").val()!='{number} slot available' ? " available_slots_singular_label='" + $("#available_slots_singular_label").val() + "'" : "") + 
		($("#available_slots_plural_label").val()!='{number} slots available' ? " available_slots_plural_label='" + $("#available_slots_plural_label").val() + "'" : "") + 
