(function($){
    $(document).ready(function() {
 	    qodefRepeater.rowRepeater.init();
 	    qodefRepeater.rowInnerRepeater.init();
	    qodefInitSortable();
    });

	var qodefRepeater = function() {
		...
		var rowRepeater = function() {
			...
			var addNewRow = function(holder) {
				...
				$addButton.on('click', function(e) {
					...
					qodefRepeater.rowInnerRepeater.addNewRowInner(new_holder);
					qodefRepeater.rowInnerRepeater.removeRowInner(new_holder);
					qodefInitSortable();
					qodefUIAdmin.qodefInitSwitch();
					qodefUIAdmin.qodefInitMediaUploader();
					qodefUIAdmin.qodefInitColorpicker();
					qodefUIAdmin.qodefInitDatePicker();
					qodefUIAdmin.qodefSelect2();
					qodefDependencyAdmin.qodefReInitOptionsDependency();
			...
			var getLastRowIndex = function(holder) {
				var $repeaterContent = holder.find('.qodef-repeater-wrapper-inner');
				var fieldsCount = $repeaterContent.find('.qodef-repeater-fields-holder').length;
		...
		var rowInnerRepeater = function() {
			...
			var addNewRowInner = function(holder) {
				...
				holder.on('click', '.qodef-inner-clone', function(e) {
					...
					var lastRowInnerIndex = $parentRow.find('.qodef-repeater-inner-fields-holder').length;
