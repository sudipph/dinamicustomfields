...
function tt_timetable($atts, $content)
{
	...
	$defaults = array(
		...
		"available_slots_singular_label" => "{number} slot available",
		"available_slots_plural_label" => "{number} slots available",
	...
	$output .= "<div class='tt_booking tt_hide'>
...
function tt_get_timetable($atts, $event = null)
{
	...
	$output = "";
	$query = "SELECT TIME_FORMAT(t1.start, '%H.%i') AS start, TIME_FORMAT(t1.end, '%H.%i') AS end, t1.tooltip AS tooltip, t1.before_hour_text AS before_hour_text, t1.after_hour_text AS after_hour_text, t1.available_places AS available_places, t4.booking_count, t1.event_hours_id AS event_hours_id, t2.ID AS event_id, t2.post_title AS event_title, t2.post_name AS post_name, t3.post_title, t3.menu_order, t5.booking_id AS current_user_booking_id FROM " . $wpdb->prefix . "event_hours AS t1 
			LEFT JOIN {$wpdb->posts} AS t2 ON t1.event_id=t2.ID 
			LEFT JOIN {$wpdb->posts} AS t3 ON t1.weekday_id=t3.ID 
			LEFT JOIN (SELECT event_hours_id, COUNT(booking_id) as booking_count FROM " . $wpdb->prefix . "event_hours_booking GROUP BY event_hours_id) AS t4 ON t1.event_hours_id=t4.event_hours_id 
			LEFT JOIN " . $wpdb->prefix . "event_hours_booking AS t5 ON t1.event_hours_id=t5.event_hours_id AND t5.user_id='" . $user_id . "' 
			WHERE 
			...
			for($j=0; $j<count($weekdays); $j++)
			{
				...
				{	
					if(tt_hour_in_array($i, (isset($event_hours_tt[$weekday_fixed_number]) ? $event_hours_tt[$weekday_fixed_number] : array()), $measure, $hours_min))
					{
						...
						$row_content .= tt_get_row_content($events, compact("events_page", "time_format", "event_layout", "global_colors", "disable_event_url", "show_booking_button", "show_available_slots", "booking_label", "available_slots_singular_label", "available_slots_plural_label", "booked_label", "unavailable_label"));
						$row_content .= '</td>';
						$row_empty = false;
					}
					else
				...
				<ul class="tt_items_list thin page_margin_top timetable_clearfix' . (isset($mode) && $mode=='12h' ? ' mode12' : '') . '">';
					...
					for($i=0; $i<$event_hours_count; $i++)
					{
						...
						if($show_booking_button!="no" && $show_available_slots!="no" && $available_slots)
						{
							...
							$available_slots_label = ($available_slots==1 ? $available_slots_singular_label : $available_slots_plural_label);
							$available_slots_label = str_replace('{number}', '<span class="count">' . $available_slots . '</span> ', $available_slots_label);
							$output .= $available_slots_label;
						...
						if($show_booking_button!="no")
						{
							...
							{
								$output .= "<div class='event_hour_booking_wrapper " . $show_booking_button . "'><a href='#' class='event_hour_booking id-" . $event_hours_tt[$weekday_fixed_number][$i]["event_hours_id"] . " booked " . $show_booking_button . "' style='" . (strlen($booked_text_color) ? " color: #" . $booked_text_color . " !important;" : "") . (strlen($booked_bg_color) ? " background-color: #" . $booked_bg_color . " !important;" : "") . "' title='" . $booked_label . "'>" . $booked_label . "</a></div>";
							}
							elseif(!$available_slots)
							{
								$output .= "<div class='event_hour_booking_wrapper " . $show_booking_button . "'><a href='#' class='event_hour_booking id-" . $event_hours_tt[$weekday_fixed_number][$i]["event_hours_id"] . " unavailable " . $show_booking_button . "' style='" . (strlen($unavailable_text_color) ? " color: #" . $unavailable_text_color . " !important;" : "") . (strlen($unavailable_bg_color) ? " background-color: #" . $unavailable_bg_color . " !important;" : "") . "' title='" . $unavailable_label . "'>" . $unavailable_label . "</a></div>";
							}
							else
							{
...
//		$available_slots_color = ($global_colors['available_slots_color']!="" && $global_colors['available_slots_color']!="FFD544" ? $global_colors['available_slots_color'] : "");
		...
		for($i=0; $i<$hours_count; $i++)
		{
			...
			if($show_booking_button!="no")
			{
				...
				{
					$available_slots_html = "<span class='available_slots id-" . $details["event_hours_id"][$i] . "'>";
					$available_slots_label = ($available_slots==1 ? $available_slots_singular_label : $available_slots_plural_label);
					$available_slots_label = str_replace('{number}', '<span class="count">' . $available_slots . '</span> ', $available_slots_label);
					$available_slots_html .= $available_slots_label;
					$available_slots_html .=
					"</span>";
				}
			}
