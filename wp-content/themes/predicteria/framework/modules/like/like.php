...
class WellExpoSelectClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_wellexpo_select_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_wellexpo_select_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'wellexpo' );
		
		if ( isset( $_COOKIE[ 'qodef-like_' . $post->ID ] ) ) {
			$class = 'qodef-like liked';
			$title = esc_html__( 'You already like this!', 'wellexpo' );
		}
...
if ( ! function_exists( 'wellexpo_select_create_like' ) ) {
	function wellexpo_select_create_like() {
		WellExpoSelectClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'wellexpo_select_create_like' );
}