...
$(document).on("click", ".organizer_sponsors_submit_1", function (event) {
    ...
    if (selected_sponsors != '') {
        ...
        jQuery.ajax({
            ...
            type: "post",
            data: {
                action: "ajax_sponsors_textarea1",
...
function remove_field_editor_box(vart, id, post_id) {
    jQuery.ajax({
        url: ajax_url,
        type: "post",
        data: {
            action: "ajax_sponsors_remove_sp",
            post_key: id,
            post_id: post_id
        },
        dataType: "json",
