...
class WellExpoSelectClassClassIconsGroupWidget extends WellExpoSelectClassWidget {
	public function __construct() {
		parent::__construct(
			'qodef_social_icons_group_widget',
			esc_html__( 'WellExpo Social Icons Group Widget', 'wellexpo' ),
			array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'wellexpo' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			...
			wellexpo_select_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
			array(
				array(
					'type'    => 'dropdown',
					'name'    => 'layout',
					'title'   => esc_html__( 'Icons Layout', 'wellexpo' ),
					'options' => array(
						''             => esc_html__( 'Default', 'wellexpo' ),
						'square-icons' => esc_html__( 'Square', 'wellexpo' ),
					)
				),
				array(
					'type'        => 'dropdown',
					'name'        => 'skin',
					'title'       => esc_html__( 'Square Icons Skin', 'wellexpo' ),
					'description' => esc_html__( 'This applies to the square layout', 'wellexpo' ),
					'options'     => array(
						''           => esc_html__( 'Dark Skin', 'wellexpo' ),
						'light-skin' => esc_html__( 'Light Skin', 'wellexpo' ),
					)
				),
				array(
					'type'    => 'dropdown',
					'name'    => 'alignment',
					'title'   => esc_html__( 'Text Alignment', 'wellexpo' ),
					'options' => array(
				...
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icons Size (px)', 'wellexpo' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'color',
	...
	public function widget( $args, $instance ) {
		...
		echo '<div class="widget qodef-social-icons-group-widget ' . esc_attr($class) . ' clearfix">';
		...
		for ( $n = 1; $n <= 6; $n++ ) {
			...
			<?php if ( ! empty( $icon_holder_html ) ) { ?>
				<a class="qodef-social-icon-widget-holder qodef-icon-has-hover" <?php echo wellexpo_select_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php wellexpo_select_inline_style( $icon_styles ) ?>
