...
$(document).on("click", ".coverimage_2", function (e) {
    console.log('start');
    e.preventDefault();
    e.stopPropagation()
    var image_frame;
...
$(document).on("click", ".organizer_sponsors_submit", function (event) {

    var that = this;
    // jQuery(this).closest('div.add_sponsors_loading').addClass('overlay');
    var selected_sponsors = jQuery('#speaker_user_id').val();
    var sponsor_post_id = jQuery('#speaker_post_id').val();
        ...
        jQuery.ajax({
            ...
            success: function (result) {
                ...
                if (result) {
                    ...
                    jQuery('#css3_animated_slider_speaker_list').append(result.htmlimage);
...
function Refresh_Image3(the_id) {
    var data = {
...
function remove_field_editor_box1(vart, id, post_id) {
    jQuery.ajax({
        url: ajax_url,
        type: "post",
        data: {
            action: "ajax_sponsors_remove_sum",
            post_key: id,
            post_id: post_id
        },
        dataType: "json",
