...
class WellExpoSelectClassSocialIconWidget extends WellExpoSelectClassWidget {
	
	public function __construct() {
		parent::__construct(
			'qodef_social_icon_widget',
			esc_html__( 'Predicteria Social Icon Widget', 'wellexpo' ),
			array( 'description' => esc_html__( 'Add social network icons to widget areas', 'wellexpo' ) )
		);
		...
		$this->params = array_merge(
			array(
				array(
					'type'  	  => 'dropdown',
					'name'  	  => 'icon_text',
					'title' 	  => esc_html__( 'Display Icon or Text', 'wellexpo' ),
					'description' => esc_html__( 'Choose whether you want to display social icon or text instead', 'wellexpo' ),
					'options' 	  => array(
						'icon' => esc_html__( 'Icon', 'wellexpo' ),
						'text' => esc_html__( 'Text', 'wellexpo' )
					)
				),
				array(
					'type'        => 'textfield',
					'name'        => 'social_text',
					'title'       => esc_html__( 'Social Text', 'wellexpo' ),
					'description' => esc_html__( 'Instert text which will describe desired social network', 'wellexpo' )
				)
			),
			wellexpo_select_icon_collections()->getSocialIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'link',
	...
	public function widget( $args, $instance ) {
		...
		?>
		<a class="qodef-social-icon-widget-holder qodef-icon-has-hover" <?php echo wellexpo_select_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php wellexpo_select_inline_style( $icon_styles ) ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>">
