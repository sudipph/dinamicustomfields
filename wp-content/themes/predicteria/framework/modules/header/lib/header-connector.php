...
namespace WellExpoSelectNamespace\Modules\Header\Lib;

/**
 * Class WellExpoSelectClassHeaderConnector
 *
...
class WellExpoSelectClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'wellexpo_select_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'wellexpo_select_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'wellexpo_select_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'wellexpo_select_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to wellexpo_select_filter_title_content_padding filter
	 *
	 * @return int
	 */
