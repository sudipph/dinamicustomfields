...
(function($) {
    ...
    common.qodefShuffleIndexes = qodefShuffleIndexes;
    ...
    */
    function qodefOnWindowLoad() {
		...
		qodefInitParallaxElements();
		qodefGlitchImages();
		qodefRowBgText();
		qodefItemsWithLoadingAnimation();
    }
    ...
    */
	...
	function qodefInitBackToTop() {
		...
		backToTopButton.on('click', function (e) {
			e.preventDefault();
			qodef.html.animate({scrollTop: 0}, qodef.window.scrollTop() / 5, 'easeInOutQuart');
		});
	}
	...
	function qodefBTTSkin() {
		var btt = $('#qodef-back-to-top'),
			footerBtm = $('.qodef-footer-bottom-holder');

		var scrollHandler = function(event) {
			if (qodef.scroll + qodef.windowHeight >= footerBtm.offset().top) {
				if (!btt.hasClass('qodef-dark')) {
					btt.addClass('qodef-dark');
				}
			} else {
				if (btt.hasClass('qodef-dark')) {
					btt.removeClass('qodef-dark');
				}
			}
		}

		if (btt.length && footerBtm.length) {
		...
		if (qodef.body.hasClass('qodef-smooth-page-transitions')) {
			...
			if (qodef.body.hasClass('qodef-smooth-page-transitions-preloader')) {
				var loader = $('body > .qodef-smooth-transition-loader.qodef-mimic-ajax'),
					uncoverSpinner = $('.qodef-uncover-spinner'),
					mainRevHolder = $('#qodef-main-rev-slider-holder');
				...
				if (uncoverSpinner.length) {
					var squareHolder = $('.qodef-uncover-square-holder'),
						square = squareHolder.find('.qodef-uncover-square');

					qodef.body.addClass('qodef-uncover-square-animating');
					square.one('animationiteration webkitAnimationIteration', function() {
						qodef.body
							.addClass('qodef-uncover-square-removed');

						var finalAnimation = function() {
							squareHolder
							.addClass('qodef-remove')
							.one(qodef.animationEnd, function() {
								uncoverSpinner.addClass('qodef-remove');
								fadeOutLoader(0, 1000);
							});
						}

						if (mainRevHolder.length) {
							mainRevHolder.find('.rev_slider').bind('revolution.slide.onloaded', function() {
								finalAnimation();
							});
						} else {
							$('.qodef-content').waitForImages(function() {
								finalAnimation();
							});
						}
					})
					...
					$(window).on('load', function() {
						fadeOutLoader(600, 'easeOutQuart');
					});
				}
			...
			if (qodef.body.hasClass('qodef-smooth-page-transitions-fadeout')) {
				...
				linkItem.on('click', function (e) {
					...
					if (
						...
						e.preventDefault();
						$('.qodef-wrapper-inner').fadeOut(600, 'easeOutQuart', function () {
                ...
                searchField.on('keyup paste', function() {
                    ...
                    keyPressTimeout = setTimeout( function() {
                        ...
                        if(searchTerm.length < 3) {
                            ...
                            var ajaxData = {
                                action: 'wellexpo_select_search_post_types',
                                term: searchTerm,
                                postType: postType
                            };
    ...
    function qodefOwlSlider() {
        ...
        if (sliders.length) {
            sliders.each(function(){
	            ...
	            if (slider.hasClass('qodef-testimonials')) {
	            	responsiveNumberOfItems1 = 1;
	 ...
	 * Init Parallax Items
	 */
	function qodefInitParallaxElements() {
	 ...
	 * Init Glitch Images
	 */
	function qodefGlitchImages() {
		var glitchImages = $('.qodef-glitch-image');

		if (glitchImages.length) {
			glitchImages.each(function() {
				var image = $(this).find('img'),
					src = image.attr('src');

				image
					.wrap('<div class="qodef-glitch-image-wrapper"></div>')
					.after('<div class="qodef-glitch-wrapper"></div>');

				var glitchWrap = image.siblings('.qodef-glitch-wrapper');

				for (var i = 1; i <= 4; i++) {
					glitchWrap
						.append(
							'<div class="qodef-glitch-el" \
	 ...
	 * Row Background Text
	 */
	function qodefRowBgText() {
		var bgTextInstances = $('.qodef-row-bg-text-holder');

		if (bgTextInstances.length) {
			var calcPosition = function(instance) {
				if (instance.hasClass('qodef-left-aligned')) {
					instance.css('left', -Math.abs(instance.parent().offset().left));
				} else {
					instance.css('right', -Math.abs(instance.parent().offset().left + instance.parent().outerWidth() - qodef.windowWidth));
				}
			}

			var	wrapChars = function(instance) {
				var title = instance.find('.qodef-row-bg-text'),	
					titleChars = title.text().trim().split("");

				title.empty();
				$.each(titleChars, function (i, singleChar) {
					title.append("<span class='qodef-char-mask'> \
									<span class='qodef-char' style='transition-delay:"+i*80+"ms'>" + singleChar + "\
								</span></span>");
				});
			}


			bgTextInstances.each(function() {
				var instance = $(this);
				instance.closest('.vc_row').css('position', 'relative');
				calcPosition(instance);
				wrapChars(instance);
			});

			if (!qodef.htmlEl.hasClass('touch')) {
				bgTextInstances.appear(function() {
	 ...
	 * Shuffle array
	 */
	function qodefShuffleIndexes(arr) {
	 ...
	 */
	function qodefItemsWithLoadingAnimation() {
		var items = $('.qodef-item-with-animation');

		if (items.length && !qodef.htmlEl.hasClass('touch')) {
			items.each(function() {
				!$(this).parent().hasClass('qodef-holder-with-animation') ? $(this).parent().addClass('qodef-holder-with-animation') : '';
			});

			var lists = $('.qodef-holder-with-animation');

			lists.each(function(){
				var list = $(this),
					items = list.find('.qodef-item-with-animation'),
					articleIndexes = [];
	...
	function qodefInitBlogPagination(){
		...
		var initMainPagFunctionality = function(thisHolder) {
			...
			if(nextPage <= maxNumPages){
				...
				var ajaxData = qodef.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'wellexpo_select_blog_load_more');
...
(function($) {
     ...
     */
    function qodefOnDocumentReady() {
        qodefBlogItemsHoverClass();
		qodefBlogItemsLoadingAnimation();
		qodefBlogBgrndLoadingAnimation();
    }    
    ...
    function qodefInitBlogListShortcodePagination(){
        ...
        var initMainPagFunctionality = function(thisHolder, pagedLink) {
            ...
            if(nextPage <= maxNumPages){
                ...
                var ajaxData = qodef.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'wellexpo_select_blog_shortcode_load_more');
    ...
    function qodefBlogItemsHoverClass() {
        var items = $('.qodef-blog-list-holder .qodef-bl-item');

		if (items.length) {
            items
                .find('.qodef-post-title')
                .on('mouseenter', function() {
                    $(this).closest('.qodef-bl-item').addClass('qodef-hovered');
                })
                .on('mouseleave', function() {
                    $(this).closest('.qodef-bl-item').removeClass('qodef-hovered');
                });
        }
    }

    function qodefBlogItemsLoadingAnimation() {
        var lists = $('.qodef-bl-masonry-boxed.qodef-bl-with-animation');

		if (lists.length && !qodef.htmlEl.hasClass('touch')) {
            lists.each(function(){
                var items = $(this).find('.qodef-bl-item');
    ...
    function qodefBlogBgrndLoadingAnimation() {
        var lists = $('.qodef-bl-standard.qodef-bl-with-bgrnd-animation');
