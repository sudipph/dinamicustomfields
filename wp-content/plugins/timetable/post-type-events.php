...
function timetable_events_init()
{
	...
	if(!get_option("timetable_event_hours_booking_table_installed"))
	{
		...
		$query = "CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . "event_hours_booking` (
			...
			UNIQUE KEY `unique_index` (`event_hours_id`, `user_id`)			
		) ENGINE = MYISAM DEFAULT CHARSET=utf8;";
		$wpdb->query($query);
...
function timetable_delete_event_bookings()
{
	...
	$query .= "DELETE FROM `" . $wpdb->prefix . "event_hours_booking` 
...
function timetable_inner_events_custom_box_side($post) 
{
	...
	$query = $wpdb->prepare("SELECT 
			eh.event_id,ehb.*,u.display_name
		FROM 
			" . $wpdb->prefix . "event_hours_booking AS ehb 
		INNER JOIN 
			" . $wpdb->users . " AS u ON(ehb.user_id=u.ID)
		INNER JOIN 
			" . $wpdb->prefix . "event_hours AS eh ON(ehb.event_hours_id=eh.event_hours_id)
		WHERE
			eh.event_id=%d 
		ORDER BY 
			ehb.booking_datetime DESC", $post->ID);
	$event_hours_booking = $wpdb->get_results($query);
	$event_hours_booking_array = array();
	for($i=0,$max=count($event_hours_booking); $i<$max; $i++)
	{
		$event_hours_booking_array[$event_hours_booking[$i]->event_hours_id][] = $event_hours_booking[$i];
	}
	...
	<ul id="event_hours_list"' . (!$event_hours_count ? ' style="display: none;"' : '') . '>';
		...
		{
			$booking_count=(isset($event_hours_booking_array[$event_hours[$i]->event_hours_id]) ? count($event_hours_booking_array[$event_hours[$i]->event_hours_id]) : 0);
			...
			if($event_hours[$i]->tooltip!="" || $event_hours[$i]->before_hour_text!="" || $event_hours[$i]->after_hour_text!="" || $event_hours[$i]->category!="" || $event_hours[$i]->available_places!="")
			{
				...
				if($booking_count)
				{
					...
					foreach($event_hours_booking_array[$event_hours[$i]->event_hours_id] as $booking)
					{
						echo '<li id="booking_id_'.$booking->booking_id.'">
								<a href="'.get_edit_user_link($booking->user_id).'">'.$booking->display_name.'</a> on ' . $booking->booking_datetime.
								'<img class="operation_button delete_button delete_booking" src="' . plugins_url('/admin/images/delete.png', __FILE__) . '" alt="del" />
							</li>';
					}
					echo '</ul>';
				}
...
function timetable_save_events_postdata($post_id) 
{
	...
	if(isset($_POST["weekday_ids"]))
	{
		...
		for($i=0; $i<$hours_count; $i++)
		{
			...
			else
			{
				...
				{
					$query = $wpdb->prepare("UPDATE `" . $wpdb->prefix . "event_hours` SET weekday_id=%s, start=%s, end=%s, tooltip=%s, before_hour_text=%s, after_hour_text=%s, category=%s, available_places=%s WHERE event_hours_id=%s", $_POST["weekday_ids"][$i],  $_POST["start_hours"][$i], $_POST["end_hours"][$i], $_POST["tooltips"][$i], $_POST["before_hour_texts"][$i], $_POST["after_hour_texts"][$i], $_POST["event_hours_category"][$i], $_POST["available_places_array"][$i], $_POST["event_hours_ids"][$i]);
				}
				else
				{
	...
	{
		$delete_delete_booking_ids_count = count($_POST["delete_booking_ids"]);
		if($delete_delete_booking_ids_count)
			$wpdb->query("DELETE FROM `" . $wpdb->prefix . "event_hours_booking` WHERE booking_id IN(" . implode(",", $_POST["delete_booking_ids"]) . ")");
	}
