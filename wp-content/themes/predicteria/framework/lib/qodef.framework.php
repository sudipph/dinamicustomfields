...
/*
   Class: WellExpoSelectClassFramework
...
class WellExpoSelectClassFramework {
	...
	private function __construct() {
		$this->qodeOptions         = WellExpoSelectClassOptions::get_instance();
		$this->qodeMetaBoxes       = WellExpoSelectClassMetaBoxes::get_instance();
		$this->qodeTaxonomyOptions = WellExpoSelectClassTaxonomyOptions::get_instance();
		$this->qodeUserOptions     = WellExpoSelectClassUserOptions::get_instance();
		$this->qodeDashboardOptions = WellExpoSelectClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( WellExpoSelectClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class WellExpoSelectClassSkinManager
 *
...
class WellExpoSelectClassSkinManager {
	...
	/**
	 * @see WellExpoSelectClassSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = SELECT_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'wellexpo' );
...
/**
 * Class WellExpoSelectClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class WellExpoSelectClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( wellexpo_select_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.qodeSCIcon = '<?php echo wellexpo_select_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.qodeSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'wellexpo' ) ?>';
		</script>
	<?php }
...
/*
   Class: WellExpoSelectClassOptions
...
class WellExpoSelectClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $wellexpo_select_global_options ) ) {
			return $wellexpo_select_global_options[ $key ];
...
/*
   Class: WellExpoSelectClassAdminPage
   A class that initializes Select Admin Page
*/
class WellExpoSelectClassAdminPage implements iWellExpoSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new WellExpoSelectClassFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iWellExpoSelectInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: WellExpoSelectClassMetaBoxes
...
/*
   Class: WellExpoSelectClassMetaBox
   A class that initializes Select Meta Box
*/
class WellExpoSelectClassMetaBox implements iWellExpoSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $wellexpo_select_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: WellExpoSelectClassTaxonomyOptions
   A class that initializes WellExpoSelectClass Taxonomy Options
*/
class WellExpoSelectClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: WellExpoSelectClassTaxonomyOption
   A class that initializes WellExpoSelectClass Taxonomy Option
*/
class WellExpoSelectClassTaxonomyOption implements iWellExpoSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new WellExpoSelectClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: WellExpoSelectClassUserOptions
   A class that initializes WellExpoSelectClass User Options
*/
class WellExpoSelectClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: WellExpoSelectClassUserOption
   A class that initializes WellExpoSelectClass User Option
*/
class WellExpoSelectClassUserOption implements iWellExpoSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new WellExpoSelectClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: WellExpoSelectClassDashboardOptions
   A class that initializes WellExpoSelectClass Dashboard Options
*/
class WellExpoSelectClassDashboardOptions {
	private static $instance;
...
/*
   Class: WellExpoSelectClassDashboardOption
   A class that initializes WellExpoSelectClass Dashboard Option
*/
class WellExpoSelectClassDashboardOption implements iWellExpoSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new WellExpoSelectClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'wellexpo_select_init_framework_variable' ) ) {
	function wellexpo_select_init_framework_variable() {
		global $wellexpo_select_global_Framework;
		
		$wellexpo_select_global_Framework = WellExpoSelectClassFramework::get_instance();
		$qodeSkinManager                 = new WellExpoSelectClassSkinManager();
		$wellexpo_select_global_Framework->setSkin( $qodeSkinManager->getSkin() );
	}
	
	add_action( 'wellexpo_select_action_before_options_map', 'wellexpo_select_init_framework_variable' );
}
?>