...
namespace WellExpoCore\CPT\Shortcodes\Testimonials;
...
class Testimonials implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						...
						array(
							...
							'value'       => array(
								esc_html__( 'Regular (2-column)', 'wellexpo-core' ) => '',
								esc_html__( 'Simplified (1-column)', 'wellexpo-core' )   => 'simplified'
							),
							'save_always' => true,
						...
						array(
							...
							'value'       => array(
								...
								esc_html__( 'Light', 'wellexpo-core' )   => 'light',
							),
							'save_always' => true
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Testimonials', 'wellexpo-core' )
						),
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
	...
	public function render( $atts, $content = null ) {
		...
		$params['layout'] = ! empty( $params['layout'] ) ? $params['layout'] : '';
        ...
        return wellexpo_core_get_cpt_shortcode_module_template_part( 'testimonials', 'testimonials', 'testimonials-' . $params['type'], '', $params );
	...
	private function getSliderData( $params ) {
		...
		$slider_data['data-number-of-items']        = ($params['type'] === 'custom' && $params['layout'] !== 'simplified') ? 2 : '1';
	...
	public function testimonialsCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['testimonials_category_title'] ) > 0 ) ? esc_html__( 'Category', 'wellexpo-core' ) . ': ' . $value['testimonials_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $testimonials_category ) ) {
				...
				if ( ! empty( $testimonials_category_title ) ) {
					$testimonials_category_title_display = esc_html__( 'Category', 'wellexpo-core' ) . ': ' . $testimonials_category_title;
				}
