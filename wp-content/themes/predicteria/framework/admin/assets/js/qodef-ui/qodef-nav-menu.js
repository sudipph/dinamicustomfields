(function($) {
	...
	function qodefGenerateSerializedString() {
		...
		var menuOptions = $('input[name="qodef_menu_options"]');
		if(menuOptions.length) {
			menuOptions.val(encodeURIComponent(dataArrayString));
		} else {
			//generate hidden input field html with serialized string value
			var hiddenMenuItem = '<input type="hidden" name="qodef_menu_options" value="'+encodeURIComponent(dataArrayString)+'">';
    ...
    function qodefUpdateIconOptions() {
        ...
        navForm.on('change', '[data-icon-pack]', function() {
	        var thisItem = $(this),
		        ...
		        iconDropdown = thisItem.parents('p').first().next('.qodef-icon-select-holder').find('select'),
