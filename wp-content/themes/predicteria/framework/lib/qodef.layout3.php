...
/*
   Class: WellExpoSelectClassMultipleImages
   A class that initializes Select Multiple Images
*/

class WellExpoSelectClassMultipleImages implements iWellExpoSelectInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$wellexpo_select_global_Framework->qodeMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="qodef-gallery-uploader">
								<a class="qodef-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'wellexpo' ); ?></a>
								<a class="qodef-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'wellexpo' ); ?></a>
							</div>
						</div>
					</div>
				</div>
...
class WellExpoSelectClassTwitterFramework implements iWellExpoSelectInterfaceRender {
	public function render( $factory ) {
		$twitterApi = WellExpoTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'wellexpo' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'wellexpo' ) : esc_html__( 'Connect with Twitter', 'wellexpo' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="qodef-page-form-section" id="qodef_enable_social_share">
			<div class="qodef-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'wellexpo' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'wellexpo' ); ?></p>
			</div>
			<div class="qodef-section-content">
				<div class="container-fluid">
					<div class="row">
...
class WellExpoSelectClassInstagramFramework implements iWellExpoSelectInterfaceRender {
	public function render( $factory ) {
		$instagram_api = WellExpoInstagramApi::getInstance();
		$message       = '';
		
		//if code wasn't saved to database
		if ( ! get_option( 'wellexpo_instagram_code' ) ) {
			...
			if ( ! empty( $_GET['code'] ) ) {
				...
				$message = esc_html__( 'You have successfully connected with your Instagram account. If you have any issues fetching data from Instagram try reconnecting.', 'wellexpo' );
		...
		$buttonText = $instagram_api->hasUserConnected() ? esc_html__( 'Re-connect with Instagram', 'wellexpo' ) : esc_html__( 'Connect with Instagram', 'wellexpo' );
		?>
		<?php if ( $message !== '' ) { ?>
			<div class="alert alert-success">
				<span><?php echo esc_html( $message ); ?></span>
		...
		<div class="qodef-page-form-section" id="qodef_enable_social_share">
			<div class="qodef-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'wellexpo' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'wellexpo' ); ?></p>
			</div>
			<div class="qodef-section-content">
				<div class="container-fluid">
					<div class="row">
...
class WellExpoSelectClassRepeater implements iWellExpoSelectInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="qodef-repeater-table-heading">
					<div class="qodef-repeater-fields-holder">
						<div class="qodef-repeater-table-cell qodef-repeater-sort"><?php esc_html_e( 'Order', 'wellexpo' ) ?></div>
						...
						<div class="qodef-repeater-table-cell qodef-repeater-remove"><?php esc_html_e( 'Remove', 'wellexpo' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="qodef-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="qodef-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = wellexpo_select_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class WellExpoSelectClassFieldAddress extends WellExpoSelectClassFieldType {
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
							...
							<button id="find" class="btn btn-primary"><?php esc_html_e( 'Place the pin on the map', 'wellexpo' ); ?></button>
...
class WellExpoSelectClassFieldIcon extends WellExpoSelectClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = wellexpo_select_icon_collections()->getIconCollectionsEmpty();
