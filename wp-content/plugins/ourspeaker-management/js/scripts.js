...
$(document).on("click", ".coverimage_3", function (e) {
    console.log('start');
    e.preventDefault();
    e.stopPropagation()
    var image_frame;
...
$(document).on("click", ".organizer_sponsors_submit_6", function (event) {

    var that = this;
    // jQuery(this).closest('div.add_sponsors_loading').addClass('overlay');
    var selected_sponsors = jQuery('#speaker_user_id_our').val();
    var sponsor_post_id = jQuery('#speaker_post_id_our').val();
    ...
    if (selected_sponsors != '') {
        ...
        jQuery.ajax({
            ...
            type: "post",
            data: {
                action: "ajax_sponsors_textarea6",
            ...
            success: function (result) {
                ...
                if (result) {
                    ...
                    jQuery('#css3_animated_slider_speaker_list6').append(result.htmlimage);
...
function Refresh_Image6(the_id) {
    var data = {
...
function remove_field_editor_box6(vart, id, post_id) {
    jQuery.ajax({
        url: ajax_url,
        type: "post",
        data: {
            action: "ajax_sponsors_remove_sp6",
            post_key: id,
            post_id: post_id
        },
        dataType: "json",
