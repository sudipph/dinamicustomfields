...
/*
   Class: WellExpoSelectClassDashboardForm
   A class that initializes Select Dashboard Container
*/
class WellExpoSelectClassDashboardForm implements iWellExpoSelectInterfaceLayoutNode, iWellExpoSelectInterfaceRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
	...
	public function render($factory) {
		...
		<form method="<?php echo esc_attr($this->form_method);?>" id="<?php echo esc_attr($this->form_id);?>" class="<?php echo esc_attr($action_class)?>" data-action="<?php echo esc_attr($this->form_action);?>">
			...
			if ( wellexpo_select_core_plugin_installed() ) {
				echo wellexpo_select_get_button_html( array(
...
/*
   Class: WellExpoSelectClassDashboardGroup
   A class that initializes WellExpoSelectClass Group Field
*/
class WellExpoSelectClassDashboardGroup implements iWellExpoSelectInterfaceLayoutNode, iWellExpoSelectInterfaceRender{
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: WellExpoSelectClassDashboardTitle
   A class that initializes Dashboard Title
*/
class WellExpoSelectClassDashboardTitle implements iWellExpoSelectInterfaceRender {
	private $name;
	private $title;
...
/*
   Class: WellExpoSelectClassDashboardField
   A class that initializes WellExpoSelectClass Front Field
*/

class WellExpoSelectClassDashboardField implements iWellExpoSelectInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class WellExpoSelectClassDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class WellExpoSelectClassDashboardFieldText extends WellExpoSelectClassDashboardFieldType {
...
class WellExpoSelectClassDashboardFieldTextArea extends WellExpoSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class WellExpoSelectClassDashboardFieldImage extends WellExpoSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
		...
		<div class="qodef-dashboard-field-holder <?php echo esc_attr($class); ?>">
			...
			<div class="qodef-dashboard-gallery-holder">
				...
				<div class="qodef-dashboard-gallery-uploader">
					<?php 
					if ( wellexpo_membership_theme_installed() ) {
						echo wellexpo_select_get_button_html( array(
							'text'      => esc_html__( 'Upload', 'wellexpo' ),
							'custom_class' => 'qodef-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="qodef-btn qodef-btn-medium qodef-btn-solid qodef-dashboard-gallery-upload"><span class="qodef-btn-text">' . esc_html__( 'Upload', 'wellexpo' ) . '</span></a>';
					} ?>
					...
					<?php if ($value !== '' && $value !== false) { ?>
						<button class="qodef-btn qodef-btn-solid qodef-dashboard-remove-image"><?php esc_html_e('Remove Media','wellexpo'); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class WellExpoSelectClassDashboardFieldGallery extends WellExpoSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class WellExpoSelectClassDashboardFieldSelect extends WellExpoSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class WellExpoSelectClassDashboardFieldDate extends WellExpoSelectClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class WellExpoSelectClassDashboardFieldIcon extends WellExpoSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
...
class WellExpoSelectClassDashboardFieldColor extends WellExpoSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class WellExpoSelectClassDashboardFieldCheckBoxGroup extends WellExpoSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class WellExpoSelectClassDashboardFieldAddress extends WellExpoSelectClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
			...
			<div class="qodef-dashboard-field-row">
				<div class="qodef-dashboard-item col-lg-<?php echo esc_attr($col_width); ?>">
					<div class="qodef-dashboard-input-holder">
						...
						<a id="reset" href="#" class="qodef-hide-field"><?php esc_html_e( 'Reset Marker', 'wellexpo' ); ?></a>
					</div>
				</div>
			</div>
		</div>
...
class WellExpoSelectClassDashboardRepeater implements iWellExpoSelectInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name        = $name;
		$this->num_of_rows = 1;
		$this->button_text = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'wellexpo' );
			...
			<?php if ($this->table_layout) { ?>
				<div class="qodef-dashboard-repeater-table-heading">
					<div class="qodef-dashboard-repeater-fields-holder">
						<div class="qodef-dashboard-repeater-table-cell qodef-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'wellexpo' ) ?></div>
						...
						<div class="qodef-dashboard-repeater-table-cell qodef-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'wellexpo' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class WellExpoSelectClassDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array()) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new WellExpoSelectClassDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'textarea':
				$field = new WellExpoSelectClassDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'date':
				$field = new WellExpoSelectClassDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new WellExpoSelectClassDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'gallery':
				$field = new WellExpoSelectClassDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new WellExpoSelectClassDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new WellExpoSelectClassDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new WellExpoSelectClassDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'checkboxgroup':
				$field = new WellExpoSelectClassDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'address':
				$field = new WellExpoSelectClassDashboardFieldAddress();
